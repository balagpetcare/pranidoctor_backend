...
export async function POST(
  ...
  try {
    ...
    const data = await getAiRegistryAdminService().toggleFailoverRule(id, enabled, auth.actor.id);
    return jsonOk(data);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to toggle failover rule';
