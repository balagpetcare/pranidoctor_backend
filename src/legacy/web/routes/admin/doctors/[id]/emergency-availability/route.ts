...
import { adminUpdateEmergency } from "@/lib/admin-doctors/doctor-admin-service";
import { doctorMutationErrorResponse } from "@/lib/admin-doctors/mutation-errors";
import { availabilityBodySchema } from "@/lib/admin-doctors/schemas";
...
export async function POST(request: Request, ctx: RouteCtx) {
  ...
  try {
    const doctor = await adminUpdateEmergency(id, parsed.data.acceptsEmergency);
    if (!doctor) {
