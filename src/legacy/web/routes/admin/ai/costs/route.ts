...
export async function GET(request: Request) {
  ...
  try {
    const data = await getAiPlatformAdminService().getCostsDashboard(parseSinceDays(request));
    return jsonOk(data);
  } catch {
    return jsonError("DATABASE_ERROR", "Failed to load AI costs dashboard", 500);
  }
}
