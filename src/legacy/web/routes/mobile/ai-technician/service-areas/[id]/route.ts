...
import { deleteDivisionServiceArea } from "@/lib/mobile-ai-technician/application-service";
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  try {
    const result = await deleteDivisionServiceArea(auth.ctx.userId, id);
    if (result.ok === "NOT_FOUND") {
      return jsonError("NOT_FOUND", "এলাকা খুঁজে পাওয়া যায়নি", 404);
    }
    if (result.ok === "NOT_EDITABLE") {
      return jsonError(
        "NOT_EDITABLE",
        "এই অবস্থায় এলাকা মুছতে পারবেন না।",
        409,
