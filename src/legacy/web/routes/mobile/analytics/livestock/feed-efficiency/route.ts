...
export async function GET(request: Request) {
  ...
  try {
    const efficiency = await getFeedAnalyticsService().getFeedEfficiency(
      auth.ctx.customerProfileId,
      parsed.data.farmRef,
      parsed.data.from,
      parsed.data.to,
    );
    return jsonOk({ efficiency });
  } catch {
    return jsonError("DATABASE_ERROR", "Could not load feed efficiency", 500);
  }
}
